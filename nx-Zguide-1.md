...
![nordix-cool-super-HDD](https://github.com/jimmykallhagen/nordix-zfs/blob/main/nordix-HDD.png)


# ZPOOL Stipe - HDD - SSD Special Vdev 
...
## **REDUNDACY**
>If you have very important data you may need to consider some form of redundancy, in my opinion you want to run HDD in stripe to be able to have a nice home system, but there are solutions to achieve this with other raid configurations in combination with l2arc, special vdev and slog in combination with the options sync=always on the dataset, gives you a zpool with rudundas and very smooth use and masks any limited performance, but it requires you to build your entire computer for this very purpose. If you instead want to create a simple but effective storage pool with simple means, I have an alternative solution to the redundancy problem. This involves partitioning your HDD into two parts, one that you run stripe on and a smaller one that you run mirror or raidz1. You can imagine that most of the media, such as Steam games libraries etc. can be downloaded again.
so you can now run it on the stripe part and then actually be able to use your HDD effectively for gaming, then have a second partition on your HDD which is then preferably run with zfs raidz1, you can then put critical data on it and then be safe
...
**Reach your system's full storage pool potential with ZFS**
 ...
 - This is my own setup for a HDD zpool, 3 HGST 8TB, used with between 20-25 thousand hours of operation,
...
> ⚠️ </br>
**_Always make sure your HDD's have good ventilation and that their mounting has vibration damping, if any of this is lacking in your setup you can drastically minimize the lifespan of your HDD's._** </br>
> 🚫 </br>
**_Never expose your HDD to shocks and make sure to never drop one while mounting, this is guaranteed to destroy it instantly._**
---
...
Instead I recommend using l2arc for HDD pools.
If you already have your system on an nvme you can create a zvol and use it as a cache for HDD (l2arc).
...
**_4️⃣_**
 - **Zpool create tank**
This will create a fast Stripe HDD pool, make sure your HDD is in good condition and that it is okay. 
...
**⚠️**⬇️
>Remember to always set ashift correctly, modern NVME SSD, SATA SDD and HDD usually always have 4k (4096) sectors, your devices will lie to you and the system and often show 512 sectors, this is to be compatible with windows, Windows still runs today with the same kernel and file system it had when it was released in 1993, therefore, as a hardware manufacturer, you must comply with these ancient specifications. 
...
- I set the name tank then I use the disk by-id I previously sorted out for my HDD and set them in a row with a space between each after tank, if you want a different name than tank then just change it, tank is however a standard name for a storage pool in ZFS
